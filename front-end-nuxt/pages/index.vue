  ...
  methods:{
    ...
    async createUser(){
      if(this.create_salary > 0 && this.create_first_name != "" && this.create_image_url != ""){
        const { data, status } = await axios.post(`http://localhost:8080/users`, {
    ...
    async deletePost(index){
      const { data, status } = await app.get(`users(${this.post_user})`);
      console.log(status);
      console.log(data);
      console.log(data.posts);
      data.posts.splice(index,1);
      console.log(data.posts);
      const second = await app.put(`users(${this.post_user})`, data);
